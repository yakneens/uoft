  ...
  int time = 0;
	...
	while((curEvent = dequeueEvent(events)) != 0){
		...
		case ProcessBegin:
			...
			//Remember when process entered ready state.
			}else{
				current->last_ready_time = time;
				enqueueProcessByPriority(&readyQueue, current);
			}
			break;
		...
		case IOBegin:
			...
			if(!isEmpty(&readyQueue)){
				...
				//Retreive process with highest priority from ready queue
		...
		case IODone:
			...
			//Place process back in ready queue according to priority.
			enqueueProcessByPriority(&readyQueue, current);
			break;
		default:
			break;
