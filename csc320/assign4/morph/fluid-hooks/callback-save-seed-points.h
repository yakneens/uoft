...

{
	...
	fname = fl_file_chooser("Save Matte Seed Points...", "*.cmd", "");
	if (fname == NULL){
		OK = 0;
	}else{
		FILE *fp;
		
		/* If a file has been successfully chosen, write the contents
		   of the scene to the file. */
		fp = fopen(fname, "w");
		fputs(imagePanel->writeBoundarySeedPoints().c_str(), fp);
		fflush(fp);
		fclose(fp);
	}
